...
export function CoinStaking({ className = '' }: CoinStakingProps) {
  ...
  const loadStakingData = async () => {
    ...
    try {
      ...
      // Load staking pools (mock data for demo)
      const pools: StakingPool[] = Object.values(COIN_TYPES).map(coinType => ({
        coinType,
        totalStaked: Math.random() * 1000000, // Mock total staked
        apr: COIN_CONFIG.stakingApr[coinType as keyof typeof COIN_CONFIG.stakingApr] || 10,
        userStaked: Math.random() * 1000, // Mock user staked amount
        userRewards: Math.random() * 50, // Mock rewards
        lockPeriod: coinType === COIN_TYPES.RISK ? 30 : coinType === COIN_TYPES.ORACLE ? 14 : 7,
        isActive: true,
      }));
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="text-center p-3 bg-purple-50 rounded-lg">
            <div className="text-lg font-bold text-purple-600">
              {stakingPools.length > 0 ? (stakingPools.reduce((sum, p) => sum + p.apr, 0) / stakingPools.length).toFixed(1) : 0}%
            </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-semibold text-blue-900">
                {getCoinMetadata(selectedPool.coinType).name} Staking
              </h4>
            ...
            {activeTab === 'stake' ? (
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex justify-between text-xs text-blue-700 mt-1">
                    <span>Available: {getAvailableBalance(selectedPool.coinType).toFixed(2)} {selectedPool.coinType}</span>
                    <span>
                      Est. rewards: +{calculateRewards(parseFloat(stakeAmount) || 0, selectedPool.apr, selectedPool.lockPeriod).toFixed(4)}
                    </span>
                  </div>
                </div>
                ...
                <button
                  onClick={handleStake}
                  disabled={!stakeAmount || parseFloat(stakeAmount) <= 0 || transactionStatus === 'pending'}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex justify-between text-xs text-blue-700 mt-1">
                    <span>Staked: {selectedPool.userStaked.toFixed(2)} {selectedPool.coinType}</span>
                ...
                <button
                  ...
                  disabled={!unstakeAmount || parseFloat(unstakeAmount) <= 0 || transactionStatus === 'pending'}
